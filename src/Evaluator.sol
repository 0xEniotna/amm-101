...
contract Evaluator {
    ...
    event constructedCorrectly(
        address erc20Address, address dummyTokenAddress, address uniFactoryAddress, address wethAddress
    );
    ...
    function ex2_showIProvidedLiquidity() public {
        ...
        (address token0, address token1) =
            address(dummyToken) < WETH ? (address(dummyToken), WETH) : (WETH, address(dummyToken));
        ...
        require(
            dummyTokenAndWethPairAsERC20.balanceOf(msg.sender) > 0, "You do not hold liquidity in the required pool"
        );
    ...
    function ex3a_getTickerAndSupply() public {
    ...
    function ex3b_testErc20TickerAndSupply() public {
    ...
    function ex4_tokenIsTradableOnUniswap() public {
        // Retrieving address of pair from library
        (address token0, address token1) = address(studentErc20[msg.sender]) < WETH
    ...
    function ex5_contractCanSwapVsEth() public {
        // Retrieving address of pair from library
        (address token0, address token1) = address(studentErc20[msg.sender]) < WETH
            ? (address(studentErc20[msg.sender]), WETH)
            : (WETH, address(studentErc20[msg.sender]));
        ...
        uint256 initBalance = studentErc20[msg.sender].balanceOf(address(studentExercice[msg.sender]));
        ...
        uint256 endBalance = studentErc20[msg.sender].balanceOf(address(studentExercice[msg.sender]));
    ...
    function ex6_contractCanSwapVsDummyToken() public {
        ...
        (address token2, address token3) =
            WETH < address(dummyToken) ? (WETH, address(dummyToken)) : (address(dummyToken), WETH);
        ...
        // Checking caller balance before executing contract
        uint256 initTokenBalance = studentErc20[msg.sender].balanceOf(address(studentExercice[msg.sender]));
        uint256 initDummyBalance = dummyToken.balanceOf(address(studentExercice[msg.sender]));
        ...
        // Checking your token balance after calling the exercice
        uint256 endTokenBalance = studentErc20[msg.sender].balanceOf(address(studentExercice[msg.sender]));
        require(initTokenBalance != endTokenBalance, "You still have the same amount of your tokens");
        uint256 endDummyBalance = dummyToken.balanceOf(address(studentExercice[msg.sender]));
    ...
    function ex7_contractCanProvideLiquidity() public {
        // Retrieving address of pair from library
        (address token0, address token1) = address(studentErc20[msg.sender]) < WETH
            ? (address(studentErc20[msg.sender]), WETH)
            : (WETH, address(studentErc20[msg.sender]));
    ...
    function ex8_contractCanWithdrawLiquidity() public {
        // Retrieving address of pair from library
        (address token0, address token1) = address(studentErc20[msg.sender]) < WETH
            ? (address(studentErc20[msg.sender]), WETH)
            : (WETH, address(studentErc20[msg.sender]));
    ...
    function readTicker(address studentAddres) public view returns (string memory) {
